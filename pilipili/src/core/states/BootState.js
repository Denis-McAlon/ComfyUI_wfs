 ...
 * to character select. Keeping it as its own state (rather than starting the FSM on
 * 'select' directly) means the flow diagram in Game.js stays honest and there's an
 * obvious home for a future title card / studio sting / save-slot check.
 *
...
export class BootState {
  ...
  enter() {
    ...
    // Straight to the roster. The transition is deferred by the FSM to the end of
    // this hook, so it's safe to request it inline here.
    ctx.goto('select');
  }
}
